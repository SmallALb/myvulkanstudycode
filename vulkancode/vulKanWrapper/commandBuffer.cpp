...
namespace FF::Wrapper {
	...
	void CommandBuffer::bindVertexBuffer(const std::vector<VkBuffer>& buffer) {
		std::vector<VkDeviceSize> offsets(buffer.size(), 0);

		vkCmdBindVertexBuffers(mCommandBuffer, 0, static_cast<uint32_t>(buffer.size()), buffer.data(), offsets.data());
	}
	...
	void CommandBuffer::draw(size_t vertextCount) {
		vkCmdDraw(mCommandBuffer, vertextCount, 1, 0, 0);
	}
